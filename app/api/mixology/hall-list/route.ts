...
function parseType(value: unknown): HallType | null { return value === "material" || value === "recipe" ? value : null; }
function isMissingRpcError(message: string): boolean { return /mixology_(item|recipe)_list|PGRST202|Could not find the function|schema cache|does not exist/i.test(message); }
function isMissingTableError(message: string): boolean { return /mixology_items|mixology_recipes/i.test(message) && /PGRST20[45]|Could not find the table|schema cache|does not exist/i.test(message); }
function coverUrl(type: HallType, id: string, updatedAt: string, hasCover: boolean): string { return hasCover ? `/api/mixology/cover?type=${type}&id=${encodeURIComponent(id)}&v=${encodeURIComponent(updatedAt)}` : ""; }
function normalizeEntry(type: HallType, row: HallListRow, optimized: boolean): Record<string, unknown> | null {
  const id = cleanText(row.id, 160); const name = cleanText(row.name, 80); if (!id || !name) return null;
  const updatedAt = cleanText(row.updated_at, 80) || new Date().toISOString(); const rawCover = cleanText(row.cover, 2_000_000); const hasCover = row.has_cover === true || row.has_cover === "true";
  const base = { id, name, authorId: cleanText(row.author_id, 160) || "anonymous", authorName: cleanText(row.author_name, 80) || "匿名调酒师", authorAvatar: cleanText(row.author_avatar, 300_000), cover: optimized ? coverUrl(type, id, updatedAt, hasCover) : rawCover, likeCount: clampCount(row.like_count), saveCount: clampCount(row.save_count), viewCount: clampCount(row.view_count), commentCount: clampCount(row.comment_count), createdAt: cleanText(row.created_at, 80) || updatedAt, updatedAt };
  if (type === "material") { const kind = cleanText(row.kind, 20); if (!(MATERIAL_KINDS as readonly string[]).includes(kind)) return null; return { ...base, kind, hook: cleanText(row.hook, 200), tags: normalizeTags(row.tags) }; }
  return { ...base, intro: cleanText(row.intro, 400), charName: cleanText(row.char_name, 80), partNames: normalizeTags(row.part_names) };
}
