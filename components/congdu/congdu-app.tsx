...
import { hydrateCongduStorage } from "@/lib/congdu-storage";
import { CongduShelf } from "./congdu-shelf";
import { CongduViewer } from "./congdu-viewer";
import type { Book } from "@/lib/congdu-types";
import {
...
} from "@/lib/congdu-appearance";

type Props = { onClose: () => void };

export default function CongduApp({ onClose }: Props) {
    ...
    const loadCustomFontFace = async (blob: Blob | null) => {
        ...
        const familyName = `AIVirtualPhoneCongduFont_${Date.now()}`;
    ...
    useEffect(() => {
        hydrateCongduStorage().then(() => setReady(true));
        ...
        <div className="absolute inset-0" style={appearanceStyle}>
            ...
            {lastBookRef.current && (
                <div style={activeBook ? undefined : hiddenViewerStyle} aria-hidden={!activeBook}>
                    <CongduViewer book={lastBookRef.current} onBack={() => setActiveBook(null)} />
                </div>
            )}
        </div>
    );
