...
} from "@/lib/congdu-storage";
import { generateAnnotationBatch, generateReadingChat, parseReadingDiscussResponse, type ReadingDiscussAction, type ReadingDiscussContext } from "@/lib/congdu-engine";
...
import { PdfPageRenderer } from "./congdu-pdf-viewer";
import { decodeTxtArrayBuffer, parsePdfPageRange, PDF_PAGES_PER_CHAPTER, parseTxtContent, parseEpubFile } from "@/lib/congdu-parser";
import type { Book, BookChapter, ReadingAnnotation, ReadingProgress } from "@/lib/congdu-types";
...
// 共读自动讨论：每翻多少页触发一次讨论
const CONGDU_DISCUSS_EVERY_PAGES = 2;
...
export function CongduViewer({ book, onBack }: Props) {
    ...
    const [annotationDialogMode, setAnnotationDialogMode] = useState<"manual" | "auto" | null>(null);
    ...
    const discussTriggeredForPageRef = useRef(-1);
            ...
            if (!isPdf && chs.length === 0) {
                ...
                if (rawFile && rawFile.size > 0) {
                    try {
                        ...
                        console.error("[Congdu] Failed to rebuild text chapters from raw file:", err);
                    }
                }
            }
            setChapters(chs);
    ...
    useEffect(() => {
        ...
        void saveChapters(book.id, nextChapters).then(() => {
            ...
            console.error("[Congdu] PDF chapter skeleton error:", err);
        });
    ...
    const buildTxtBatchRequest = useCallback((size: number, mode: "manual" | "auto" | "auto-current"): AnnotationBatchRequest | null => {
    ...
    const getPdfBatchWindow = useCallback((size: number, mode: "manual" | "auto" | "auto-current") => {
    ...
    const buildPdfBatchRequest = useCallback((size: number, mode: "manual" | "auto" | "auto-current", refs: ParagraphRef[] = paragraphRefs): AnnotationBatchRequest | null => {
    ...
    const materializeBatchRequest = useCallback(async (size: number, mode: "manual" | "auto" | "auto-current"): Promise<AnnotationBatchRequest | null> => {
    ...
    const executeBatchAnnotation = useCallback(async (request: AnnotationBatchRequest, options?: { force?: boolean }) => {
        ...
        try {
            ...
            console.error("[Congdu] Annotation error:", err);
    ...
    const openAnnotationDialog = (mode: "manual" | "auto") => {
    ...
    useEffect(() => {
        ...
        void ensurePdfPageRangeParsed(chunkStart, chunkEnd).catch((err) => {
            console.error("[Congdu] PDF lazy parse error:", err);
        });
    ...
    }, [annotations, book.title, chapterIndex, chapters, currentChapter?.title, isPdf, pdfCurrentPage, txtPage, txtPages]);    // Chat send — parse AI response like chat-room does
    const handleSend = async () => {
        ...
        try {
            ...
            console.error("[Congdu] Chat error:", err);
        } finally {
            setChatting(false);
        }
    };
    ...
    // ── 共读自动讨论：每翻 CONGDU_DISCUSS_EVERY_PAGES 页触发一次 ──
    const triggerDiscussForCurrentPage = useCallback(async (silent = false) => {
        if (!companionId || chatting || generating) return;
        const session = getSession();
        if (!session) return;

        // Determine current page marker
        const currentPageMarker = isPdf ? pdfCurrentPage : `${chapterIndex}:${txtPage}`;
        if (!silent && discussTriggeredForPageRef.current === currentPageMarker) return;
        if (silent) discussTriggeredForPageRef.current = currentPageMarker;
        ...
        try {
            ...
            console.error("[Congdu] Auto discuss error:", err);
        }
    }, [book, buildDiscussContext, chapters, chatting, companionId, ensurePdfPageRangeParsed, generating, getSession, isPdf, pdfCurrentPage, pdfTotalPages, txtPage, chapterIndex, applyDiscussActions]);

    // 翻页计数 → 每 N 页触发讨论
    const countPageTurn = useCallback(() => {
        if (discussTriggeredForPageRef.current !== -1) {
            pageTurnCountRef.current += 1;
            if (pageTurnCountRef.current >= CONGDU_DISCUSS_EVERY_PAGES) {
                pageTurnCountRef.current = 0;
                void triggerDiscussForCurrentPage(true);
            }
        } else {
            // First page: mark as triggered to avoid discussing on entry
            discussTriggeredForPageRef.current = isPdf ? pdfCurrentPage : `${chapterIndex}:${txtPage}`;
        }
    }, [chapterIndex, isPdf, pdfCurrentPage, txtPage, triggerDiscussForCurrentPage]);

    // Listen to page changes → count turns
    const lastPageKeyRef = useRef<string | null>(null);
    useEffect(() => {
        const key = isPdf ? `pdf:${pdfCurrentPage}` : `txt:${chapterIndex}:${txtPage}`;
        if (lastPageKeyRef.current !== null && lastPageKeyRef.current !== key) {
            countPageTurn();
        }
        lastPageKeyRef.current = key;
    }, [chapterIndex, countPageTurn, isPdf, pdfCurrentPage, txtPage]);
        ...
        <div className="reading-app-surface absolute inset-0 z-[100] flex flex-col bg-[var(--c-page-body-bg)]" data-immersive={immersive} style={{ paddingTop: "var(--page-header-safe-top, 48px)" }}>
            ...
            {showCharPicker && (
                <div
                    ...
                    style={{ minWidth: 160, bottom: "calc(30% + 8px)" }}
                    onPointerDown={(e) => e.stopPropagation()}
                    ...
                    <div className="chat-contact-list" style={{ padding: "4px 6px" }}>
                        {enrichedContacts.map(c => (
                            <div
                                ...
                                onClick={() => { setCompanionId(c.characterId); closeCharPicker(); generatedBatchesRef.current.clear(); refreshChatMessages(); }}
                            >
            ...
            {/* ═══ 分栏主体：上 70% 阅读 + 下 30% 聊天 ═══ */}
            <div className="flex-1 min-h-0 flex flex-col">
                {/* 阅读区 70% */}
                <div
                    ref={scrollRef}
                    className="relative min-h-0 flex-[0_0_70%] px-4 pt-1 pb-3 overflow-hidden"
                    data-ui="body"
                ...
                >
                    ...
                    {isPdf ? (
                        ...
                        <div className="reading-debug-card">
                            ...
                            <div className="reading-debug-hint">这本书在本地 IndexedDB 里的章节数据已为空，不是分页问题。</div>
                        </div>
                    ) : !currentChapter ? (
                        <div className="reading-debug-card">
                            <div className="reading-debug-title">TXT 数据自检</div>
                            ...
                            <div className="reading-debug-line">当前章节索引：{chapterIndex}</div>
                            <div className="reading-debug-hint">章节数据存在，但当前索引取不到正文。</div>
                        </div>
                    ) : (
                        <>
                            <div
                ...
                {/* 页码（悬浮在阅读区右下） */}
                <span className="reading-immersive-page" style={{ top: "calc(70% - 26px)" }}>
                    {isPdf ? `${pdfCurrentPage}/${pdfTotalPages || "?"}` : `${txtDisplayedPage}/${txtTotalPages}`}
                </span>

                {/* ── 聊天区 30% ── */}
                <div className="flex-1 min-h-0 flex flex-col border-t-2" style={{ borderColor: "var(--reading-warm-brown, #c9a06a)", background: "var(--c-panel, #fffef7)" }}>
                    {/* 聊天头：角色选择 + 标题 */}
                    <div className="flex items-center gap-2 px-3 py-1.5" style={{ borderBottom: "1px solid rgba(200,180,150,.3)" }}>
                        <button
                            type="button"
                        ...
                        <span className="ts-13 font-medium flex-1 truncate" style={{ color: "var(--reading-warm-ink, #111)" }}>
                            {companion ? `和${companion.name}讨论中` : "选择陪读角色"}
                        </span>
                        <span className="ts-11" style={{ color: "var(--reading-warm-ink-tertiary, #999)" }}>
                            每翻 {CONGDU_DISCUSS_EVERY_PAGES} 页自动讨论
                        </span>
                    </div>
                    ...
                    <div
                        ...
                        onClick={() => { if (activeMessageId || readingMessageMenu) closeReadingMessageMenu(); }}
                    >
                        {chatMessages.length === 0 && (
                            <div className="text-center ts-12 text-[var(--c-icon)] py-3">
                                {companion ? `翻几页，${companion.name}会来和你讨论这段剧情` : "先选择一位陪读角色"}
                            </div>
                        )}
                        {chatMessages.map(msg => (
                            <div key={msg.id} className="chat-msg-wrapper mb-1" data-role={msg.role}
                            ...
                            >
                                <div className={`chat-bubble-role-${msg.role} rounded-lg ${msg.mediaType ? "chat-bubble-media" : "max-w-[85%]"} break-words relative`}
                        ...
                        {chatting && <div className="ts-12 text-[var(--c-icon)] py-1">{companion?.name} 正在思考...</div>}
                    </div>

                    {/* 输入栏 */}
                    <div className="flex items-center gap-2 px-2 py-1.5" style={{ borderTop: "1px solid rgba(200,180,150,.3)" }}>
                        <input
                            value={chatInput}
                            onChange={e => setChatInput(e.target.value)}
                            onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                            placeholder="和角色讨论这段剧情..."
                            className="ui-input flex-1"
                            disabled={chatting || !companionId}
                        />
                        <button
                            onClick={handleSend}
                            disabled={!chatInput.trim() || chatting || !companionId}
            ...
            {activeReadingMenuMessage && readingMessageMenu && (
                ...
                >
                    <button
                        type="button"
                        onClick={() => { copyToClipboard(activeReadingMenuMessage.content); closeReadingMessageMenu(); }}
                        className="ctx-menu-btn"
                    >复制</button>
                    <button
                        type="button"
                    ...
                    <button
                        type="button"
                        onClick={() => { deleteChatMessage(activeReadingMenuMessage.id); setChatMessages((prev) => prev.filter((msg) => msg.id !== activeReadingMenuMessage.id)); closeReadingMessageMenu(); }}
                        className="ctx-menu-btn ctx-menu-btn-danger"
                    >删除</button>
                </div>
            )}
            ...
            {editingDiscussMessage && (
                <ContentDialog
                    ...
                    onCancel={() => { setEditingDiscussMessage(null); setEditingDiscussContent(""); }}
                >
            ...
            {/* 底部进度条（固定在阅读区底部上方） */}
            <footer className="reading-footer" style={{ position: "absolute", bottom: "30%", left: 0, right: 0, background: "transparent" }}>
